...
public class StorageUtil {
    ...
    public static String getCache() {
        return Library.context.getCacheDir() + "/" + Library.app_identity + "-" + System.currentTimeMillis();
    }

    public static String getTemp() {
        return Library.context.getCacheDir() + "/temp/" + Library.app_identity + "-" + System.currentTimeMillis();
    }

    public static String getStorageDirectory() {
        return Environment.getExternalStorageDirectory() + "/." + Library.app_identity;
    }
    ...
    public static String getStorage(String content, long id, boolean isDirectory) {
        ...
        external += "/." + Library.app_identity + "/Content/" + content + id;
    ...
    public static void cleanAll() {
        try {
            if (new File(Library.context.getCacheDir() + "/").exists())
                FileUtils.cleanDirectory(new File(Library.context.getCacheDir() + "/"));
            String external = Environment.getExternalStorageDirectory().getPath();
            if (new File(external + "/." + Library.app_identity + "/Content/").exists())
                FileUtils.cleanDirectory(new File(external + "/." + Library.app_identity + "/Content/"));
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public static String[] listFiles(String path, String suffix) {
        if (path.startsWith(ROOT_ASSET)) {
            ...
            AssetManager assetManager = Library.context.getAssets();
    ...
    public static void cleanCache() throws IOException {
        FileUtils.cleanDirectory(Library.context.getCacheDir());
    }
